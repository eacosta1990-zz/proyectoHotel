...
public class FormaPagoDao {
	...
	public FormaPago traerFormaPago(int id) throws HibernateException {
		FormaPago objeto = null;
		try {
			iniciaOperacion();
			objeto = (FormaPago) session.get(FormaPago.class, id);
		} finally {
			session.close();
		}
		return objeto;
	...
	public List<FormaPago> traerFormaPago() throws HibernateException {
		...
		try {
			iniciaOperacion();
			lista = session.createQuery("from FormaPago c").list();
